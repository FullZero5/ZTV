...
<template>
  <div :data-collapsed="isCollapsed" class="group flex flex-col gap-4 py-2 data-[collapsed=true]:py-2">
    ...
    <Card class="shadow-none" v-if="!isShowHelp&&!isCollapsed">
        <CardHeader class="p-4 pb-0">
          <CardTitle class="text-sm">
            Помощь
          </CardTitle>
          <CardDescription>
            Что бы смотреть ТВ каналы загрузите файл M3U, который содержит ссылки на эти каналы, его можно найти у
            энтузиастов
            которые поддерживают их в актуальном рабочем формате
            например на сайте 
            <a  href="https://pikabu.ru/story/novosti_loganet_iptv__bolee_600_telekanalov_besplatno_02072025_12912419?utm_source=linkshare&utm_medium=sharing&utm_campaign=mobile_native_share"
              target="_blank" rel="noopener noreferrer">
            <Button variant="link" 
             >НАПРИМЕР</Button></a>
            Вы можете его загрузить сюда, доступно простое управление плейлистом, добавление и удаление каналов
          </CardDescription>
        </CardHeader>
        <CardContent class="grid gap-2.5 p-4">
          <div class="svg-container">
            <svg viewBox="0 0 144 141" fill="none" xmlns="http://www.w3.org/2000/svg" class="responsive-svg">
...
<style scoped>
.svg-container {
    width: 50%; /* или нужная вам ширина */
    overflow: hidden; /* скрыть любые части которые выходят за пределы */
  }

  .responsive-svg {
    width: 50%; /* чтобы SVG заполнял контейнер */
